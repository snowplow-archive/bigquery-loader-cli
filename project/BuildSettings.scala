...
object BuildSettings {
  ...
  lazy val basicSettings = Seq[Setting[_]](
    ...
    version               :=  "0.1.0",
    description           :=  "Uploads Snowplow enriched events from local storage to BigQuery",
    scalaVersion          :=  "2.10.1",
  ...
  lazy val scalifySettings = Seq(sourceGenerators in Compile <+= (sourceManaged in Compile, version, name, organization) map { (d, v, n, o) =>
    ...
    IO.write(file, """package com.snowplowanalytics.snowplow.bigquery.loader.generated
      |object Settings {
      |  val organization = "%s"
      |  val version = "%s"
      |  val name = "%s"
